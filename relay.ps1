...
if (-not $existingScope) {
    Write-Host "[ACTION] Creating DHCP scope for Node..." -ForegroundColor Yellow
    ...
    Write-Host "[INFO] DHCP scope and options created successfully." -ForegroundColor Green
} else {
    Write-Host "[INFO] DHCP scope for Node (192.168.99.80/28) already exists." -ForegroundColor Cyan
}

# --- NEW: Create DHCP Reservation for Node MAC Address ---
$reservation = Get-DhcpServerv4Reservation -ScopeId $nodeScopeId -IPAddress "192.168.99.82" -ErrorAction SilentlyContinue
if (-not $reservation) {
    Write-Host "[ACTION] Creating DHCP reservation for Node (192.168.99.82 -> 08002791C011)..." -ForegroundColor Yellow
    Add-DhcpServerv4Reservation -ScopeId $nodeScopeId `
        -IPAddress "192.168.99.82" `
        -ClientId "08002791C011" `
        -Name "Node" `
        -Description "Node NIC MAC Reservation" | Out-Null
    Write-Host "[INFO] DHCP reservation created successfully." -ForegroundColor Green
} else {
    Write-Host "[INFO] DHCP reservation for 192.168.99.82 already exists." -ForegroundColor Cyan
}
...
Write-Host "  - DHCP server for Node ready (reserves 192.168.99.82 for 08002791C011)" -ForegroundColor Green
